    ...
    <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex gap-8">
        <nuxt-link v-for="link in links" :key="link.path" :to="link.path"
          ...
          {{ link.label }}
          <span
            class="absolute -bottom-[25px] left-0 w-full h-[2px] bg-black scale-x-0 group-[.router-link-exact-active]:scale-x-100 transition-transform"></span>
        </nuxt-link>
      </nav>
    </div>

    <transition name="menu">
      <div v-if="open" class="fixed inset-0 top-16 z-40 bg-white border-t border-zinc-100 flex flex-col md:hidden">
        <nav class="relative flex flex-col">
          <nuxt-link v-for="(link, index) in links" :key="link.path" :to="link.path" @click="open = false"
            ...
            <span
              class="text-xs font-bold tracking-[.3em] uppercase text-zinc-500 group-[.router-link-exact-active]:text-black">
              {{ link.label }}
            </span>
          </nuxt-link>
...
.menu-leave-to {
  ...
  /* Minimal slide for a clean console look */
}
